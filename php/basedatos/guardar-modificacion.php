<?php
    ...
    // http://localhost:8080/dashboard/basedatos/guardar-modificacion.php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($id === false || empty($nombre) || empty($correo)) {
            header("Location: error-v2.php?mensaje=Datos inválidos.");
            exit();
        }
        ...
        try {
            ...
            // Manejo de errores
            header("Location: error-v2.php?mensaje=Error al modificar el registro.");
            exit();
        }
    } else {
        // Manejo si no es un POST
        header("Location: error-v2.php?mensaje=Acceso no permitido.");
        exit();
    }
?>